...
class Executor():
    ...
    def __init__(self, sim, anc_old=True):
        self._set_sim(sim)
        self._register_modules()
        self.anc_old = anc_old
        self.info = []
    ...
    def run(self, pg, debug=False):
        ...
        for idx, m in enumerate(pg):
            if m == 'objects':
                sys.last_type = 'objects'
            elif m == 'events':
                sys.last_type = 'events'
    ...
    def unique(self, input_list):
        ...
        - return: object / event
        """
        if type(input_list) is not list or len(input_list) != 1:
            self.info.append(input_list)
            return 'error'
        else:
            return input_list[0]
    ...
    def filter_ancestor_old(self, events, event):
        """
    ...
    def filter_counterfact(self, events, remove_obj):
        ...
        outputs = []
        for e in events:
            if 'object' in e.keys() and set(e['object']) in cf_col_pairs:
                outputs.append(e)
        return outputs
    
    def filter_ancestor(self, events, event):
        if self.anc_old:
            return self.filter_ancestor_old(events, event)
        return self.filter_ancestor_new(events, event)

    def filter_ancestor_new(self, events, event):
        """
        Filter all ancestors of the input event in the causal graph
        - args: events(list), event(dict)
        - return: events(list)
        ...
        for e in events:
            ...
            if e in all_causes:
                ...
                for obj in e['object']:
                    ...
                    if event in self.filter_counterfact(events, obj):
                        flag = False                
                        break
                if flag:
                    output.append(e)
