...
def main(unused_argv):
    ...
    with open('./clevrer_monet_latents/train.json', "rb") as f:
        questions_data = json.load(f)
    questions_data = {int(k): v for k, v in questions_data.items()}
    print('training data loaded')
    with open('./clevrer_monet_latents/valid.json', "rb") as f:
        questions_data_test = json.load(f)
    questions_data_test = {int(k): v for k, v in questions_data_test.items()}
    ...
    variables_to_restore = [var for var in tf.global_variables() if var.name.startswith('transformer') and 'Adam' not in var.name]
